...
- Keep separate `.env.api`, `.env.web`, `.env.worker`, `.env.scheduler`, and optional `.env.realtime` files.
- Add a key to the smallest runtime schema that needs it; do not create a global unscoped env object.
- Optional S3/MinIO credentials belong only in server runtime files (`.env.api` and `.env.worker`); use `createStorageConfig` and never expose them to the browser.
- Realtime ticket and publisher secrets belong only in `.env.realtime`; validate them through `createRealtimeConfig`.
